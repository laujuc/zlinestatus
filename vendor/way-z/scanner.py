...
class Arg:
    ...
    def zig_struct_field(self) -> ZigStruct.Field:
        if self.type == 'new_id':
            if self.interface:
                interface = self.parent.interface.protocol.interfaces[self.interface].zig_type()
            ...
            return ZigStruct.Field(self.name, qm + self.zig_type(obj_use_ptr=True), comment=self.summary)
        # if self.summary:
            ...
            case 'int' | 'uint':
                if e := self.enum:
                    ...
                    if len(parts) == 2:
                        ...
                        return '.'.join(title_case(x) for x in [interface.name, enum.name])
        ...
        match self.type:
            case 'constructor':
                creates_interface = next(
                    (arg.interface for arg in self.args if arg.type == 'new_id' and arg.interface),
                    None,
                )
...
class Interface:
    ...
    def interface_impl(self) -> str:
        ...
        if self.events:
            ...
            val.fields.append(ZigStructInit.Field('event_signatures', f'&.{{{event_signatures}}}'))
            event_names = ''.join(f'"{event.name}",' for event in self.events.values())
            val.fields.append(ZigStructInit.Field('event_names', f'&.{{{event_names}}}'))

        if self.requests:
            request_names = ''.join(f'"{req.name}",' for req in self.requests.values())
            val.fields.append(ZigStructInit.Field('request_names', f'&.{{{request_names}}}'))
